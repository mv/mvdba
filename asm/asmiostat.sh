...
#
# Ref: metalink 437996.1
#      https://metalink2.oracle.com/metalink/plsql/f?p=130:3:8227951168393627762::::p3_database_id,p3_docid,p3_black_frame,P3_SHOW_HEADER,p3_show_help:NOT,437996.1,0,0,0
...
typeset usage="
    ...
    Output: DiskPath - Path to ASM disk
...
#
if [[ -z $diskgroup ]]
then query="select group_number, disk_number, name, path, reads, writes, read_errs, write_errs, read_time, write_time, bytes_read, bytes_written from v\$asm_disk_stat where group_number>0 order by group_number, disk_number;"
else query="select group_number, disk_number, name, path, reads, writes, read_errs, write_errs, read_time, write_time, bytes_read, bytes_written from v\$asm_disk_stat where group_number=(select group_number from v\$asm_diskgroup_stat where name=regexp_replace('$diskgroup','^\+','')) order by group_number, disk_number;"
fi
...
if ! (print "$version<$minversion" | bc >/dev/null 2>&1)
then print "$0 requires Oracle Database Release $minversion or later"
     exit 1
fi
...
function drainOutput {
    ...
    while :
    do read -p output || fatalError 'Read from co-process failed [$0]'
...
awk '
BEGIN   { firstSample=1 }
/^EOL$/ { firstSample=0; firstLine=1; next }
{
    path=$4
    if (path ~ /^ *$/) next
