rem $dba/privsdt.sql
rem
rem Called by $dba/privs.sql to print the Direct and Public Table Grants for the
rem specified user or role
rem
...
declare
	...
	lv_gte			sys.dba_tab_privs.grantee%TYPE;
	...
	lv_grantable		sys.dba_tab_privs.grantable%TYPE;
	last_gte		sys.dba_tab_privs.grantee%TYPE;
	last_owner		sys.dba_tab_privs.owner%TYPE;
	last_table_name		sys.dba_tab_privs.table_name%TYPE;
...
set heading off
select chr(10)||
    '         (In PRIVILEGE column, D=Delete, I=Insert, S=Select, U=Update)'||
    chr(10)
	from dual;
set heading on
set pagesize 9999
ttitle 'Direct table grants to &1 in '_NAME skip 2
select tb_owner owner, tb_name table_name, priv privilege, gtbl adm
	from priv_temp
	where gte = UPPER('&1') and UPPER('&1') <> 'PUBLIC'
	order by 1, 2;
ttitle 'Direct table grants to PUBLIC in '_NAME skip 2
select tb_owner owner, tb_name table_name, priv privilege, gtbl adm
	from priv_temp
	where gte = 'PUBLIC'
	order by 1, 2;
