#!/usr/bin/perl
#
# $Id: dump_table.pl 6 2006-09-10 15:35:16Z marcus $
# $Revision: 137 $  $Author: marcus $
#
# dump_results()
...
sub usage()
{
    ...
    Usage: $0 SID tab_name user [<password>]
        ...
        Generates
            ...
            tab_name.ins.sql     INSERT/SELECT on table
            tab_name.ins.prc.sql INSERT/SELECT using PL/SQL block.
...
sub make_ins_sel()
{
    ...
    open FILE, "> ${tab}.ins.sql"
        or die "Cannot open ${tab}.ins.sql : $! \n";

    print FILE fmt($code);

    close FILE
        or die "Cannot close ${tab}.ins.sql : $! \n";
...
sub make_ins_sel_pl()
{
    ...
    open FILE, "> ${tab}.ins.prc.sql"
        or die "Cannot open ${tab}.ins.prc.sql : $! \n";

    print FILE fmt($code);

    close FILE
        or die "Cannot close ${tab}.ins.prc.sql : $! \n";
}
