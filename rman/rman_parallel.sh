...
run {
    ...
    CONFIGURE DEVICE TYPE disk PARALLEL 4;

    allocate channel t1 type disk format '${DST}/%d_%I_%Y-%M-%D_set-%s-%p-%c_full.rman' maxpiecesize 4G;
    ...
    allocate channel t2 type disk format '${DST}/%d_%I_%Y-%M-%D_set-%s-%p-%c_ctrl.rman' maxpiecesize 4G ;
    backup 
